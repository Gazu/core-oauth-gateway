...
describe("OAuthAuditService", () => {
  it("persists a correlated event and logs STARTED followed by SUCCESS", async () => {
    ...
    expect(auditEventPersistenceRow(persisted[0])).toMatchObject({
      reason_code: null,
  ...
  it("persists functional reason and root cause as first-class event fields", async () => {
    const logger = loggerSpies();
    const persisted: AuditEvent[] = [];
    const service = new OAuthAuditService({
      logger,
    ...
    expect(auditEventPersistenceRow(persisted[0])).toMatchObject({
      reason_code: "invalid_client_assertion",
      root_cause_code: "assertion_expired",
      event_payload: persisted[0]
    });
    expect(logger.info).toHaveBeenNthCalledWith(
      1,
      "Audit event started",
  ...
  it("rejects a root cause without a failure reason", async () => {
    const persist = vi.fn(async () => undefined);
    const service = new OAuthAuditService({
      logger: loggerSpies(),
      persist
    ...
    expect(result.failureReasonCode).toBe("audit_validation_failed");
    expect(persist).not.toHaveBeenCalled();
  });

  it("rejects failure classification on a successful event", async () => {
    const persist = vi.fn(async () => undefined);
    const service = new OAuthAuditService({
      logger: loggerSpies(),
      persist
